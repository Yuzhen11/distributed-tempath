        ...
        if (step_num() == 1)
        {
            ...
            if (id == src) 
            {
                ...
                for (int i = 0; i < value().neighbors.size(); ++ i)
                {
                    ...
                    while (it != value().v_time[i].end())
                    {
                    	send_message(value().neighbors[i], make_pair(it->first, it->first+it->second));
                    	it ++;
                    }
            ...
            for (int i = 0; i < messages.size(); ++ i)
            {
            	...
            	if (it == s.end() || it->second > p.second)
            	{
            	    ...
            	    for (int j = 0; j < value().neighbors.size(); ++ j)
            	    {
            	    	...
            	    	send_message(value().neighbors[j], make_pair(p.first, it2->first+it2->second));
